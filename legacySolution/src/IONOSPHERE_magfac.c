/* Bilinear Interpolation Procedure for
   Magnetospheric Field-Aligned Current (FAC) System */

/* This subroutine obtains the required radial components
   of the magnetospheric field-aligned current (FAC) systems
   used as sources in ionosphere model.  Bilinear interpolation
   is used to map the radial component of the FAC resulting
   from the magnetosphere calculation to the two-dimensional
   uniform mesh in the co-latitude and longitude coordinate
   system (Theta, Psi) used in the ionosphere model.
   ...
   JR         Source arrray containing interpolated
              radial component of FAC (ionosphere calculation)
...
void IONOSPHERE_magfac(JR, Theta, Psi, X, Y, Z, Radius, nTheta, nPsi, Jmag_x,
                       Jmag_y, Jmag_z, Jmag_R, Thetamag, Psimag, Xmag, Ymag,
                       Zmag, nmag, InMagTri, MagTriNode1, MagTriNode2,
                       MagTriNode3, ntrimag, ireinit) double **JR,
    **Theta, **Psi, **X, **Y, **Z, Radius;
double *Thetamag, *Psimag, *Xmag, *Ymag, *Zmag, *Jmag_x, *Jmag_y, *Jmag_z,
    *Jmag_R;
int **InMagTri, *MagTriNode1, *MagTriNode2, *MagTriNode3;
int nTheta, nPsi, nmag, ntrimag, ireinit;
{
  int i, j, k, n, itrimag;
  double x1, x2, x3, y1, y2, y3, det0, det1, det2, det3, f1, f2, f3, a, b, c, x,
      y;
  double z1, z2, z3, z;
  ...
  for (j = 1; j <= nPsi; j++) {
    for (i = 1; i <= nTheta; i++) {
      ...
      if (ireinit == 0 && (Theta[i][j] <= 0.45 * IONOSPHERE_PI ||
        ...
        f1 = Jmag_R[MagTriNode1[itrimag]];
        f2 = Jmag_R[MagTriNode2[itrimag]];
        f3 = Jmag_R[MagTriNode3[itrimag]];

        det0 = x2 * y3 - y2 * x3 - x1 * y3 + y1 * x3 + x1 * y2 - y1 * x2;
        a = (x2 * y3 - y2 * x3) * f1 / det0 + (y1 * x3 - x1 * y3) * f2 / det0 +
            (x1 * y2 - y1 * x2) * f3 / det0;
        b = (y2 - y3) * f1 / det0 + (y3 - y1) * f2 / det0 +
            (y1 - y2) * f3 / det0;
        c = (x3 - x2) * f1 / det0 + (x1 - x3) * f2 / det0 +
            (x2 - x1) * f3 / det0;
        JR[i][j] = a + b * x + c * y;

      } else if (ireinit == 0 && ((Psi[i][j] >= 0.05 * IONOSPHERE_PI &&
        ...
        a = (x2 * z3 - z2 * x3) * f1 / det0 + (z1 * x3 - x1 * z3) * f2 / det0 +
            (x1 * z2 - z1 * x2) * f3 / det0;
        b = (z2 - z3) * f1 / det0 + (z3 - z1) * f2 / det0 +
            (z1 - z2) * f3 / det0;
        c = (x3 - x2) * f1 / det0 + (x1 - x3) * f2 / det0 +
            (x2 - x1) * f3 / det0;
        JR[i][j] = a + b * x + c * z;
        ...
        a = (y2 * z3 - z2 * y3) * f1 / det0 + (z1 * y3 - y1 * z3) * f2 / det0 +
            (y1 * z2 - z1 * y2) * f3 / det0;
        b = (z2 - z3) * f1 / det0 + (z3 - z1) * f2 / det0 +
            (z1 - z2) * f3 / det0;
        c = (y3 - y2) * f1 / det0 + (y1 - y3) * f2 / det0 +
            (y2 - y1) * f3 / det0;
        JR[i][j] = a + b * y + c * z;
        ...
        for (n = 1, itrimag = 0; n <= ntrimag && itrimag < 1; ++n) {
          ...
          if (x >= xmin && x <= xmax && y >= ymin && y <= ymax && z * z1 >= 0 &&
              z * z2 >= 0 && z * z3 >= 0) {
        ...
        for (n = 1, itrimag = 0; n <= ntrimag && itrimag < 1; ++n) {
          ...
          if (x >= xmin && x <= xmax && z >= zmin && z <= zmax && y * y1 >= 0 &&
              y * y2 >= 0 && y * y3 >= 0) {
        ...
        for (n = 1, itrimag = 0; n <= ntrimag && itrimag < 1; ++n) {
          ...
          if (y >= ymin && y <= ymax && z >= zmin && z <= zmax && x * x1 >= 0 &&
              x * x2 >= 0 && x * x3 >= 0) {
