...
int main() {
  ...
  double **PHI, **JR, **Theta, **Psi, **X, **Y, **Z, **Sigma0, **SigmaH,
      **SigmaP, **SigmaThTh, **SigmaThPs, **SigmaPsPs, **dSigmaThTh_dTheta,
      **dSigmaThPs_dTheta, **dSigmaPsPs_dTheta, **dSigmaThTh_dPsi,
      **dSigmaThPs_dPsi, **dSigmaPsPs_dPsi;
  double **ETh, **EPs, **Ex, **Ey, **Ez, **Jx, **Jy, **Jz, **Bx, **By, **Bz,
      **Ux, **Uy, **Uz;
  double *Thetamag, *Psimag, *Xmag, *Ymag, *Zmag, *Umag_x, *Umag_y, *Umag_z,
      *Bmag_x, *Bmag_y, *Bmag_z, *Jmag_x, *Jmag_y, *Jmag_z, *Jmag_R;
  double Radius, rho, p, B1x, B1y, B1z, q, Jll, fhem;
  ...
  for (i = 1; i <= 22; ++i)
    fscanf(input_file, "%[^\n]\n", buffer);
  fscanf(input_file, "%[^=] %1c %d %[^=] %1c %d", buffer, &symbol, &nmag,
  ...
  for (i = 1; i <= nmag; ++i) {
    fscanf(input_file,
           ...
           "%lg %lg %lg %lg",
           &Xmag[i], &Ymag[i], &Zmag[i], &Radius, &rho, &Umag_x[i], &Umag_y[i],
           &Umag_z[i], &p, &B1x, &B1y, &B1z, &Bmag_x[i], &Bmag_y[i], &Bmag_z[i],
           &q, &Jmag_x[i], &Jmag_y[i], &Jmag_z[i], &Jll);
    /* fscanf(input_file,
  ...
  for (i = 1; i <= ntrimag; ++i) {
    fscanf(input_file, "%d %d %d", &MagTriNode1[i], &MagTriNode2[i],
           &MagTriNode3[i]);
  } /* endfor */
  fclose(input_file);
  ...
  ireinit = 1;
  IONOSPHERE_magdim(Thetamag, Psimag, Xmag, Ymag, Zmag, Jmag_x, Jmag_y, Jmag_z,
                    Jmag_R, Bmag_x, Bmag_y, Bmag_z, nmag, MagTriNode1,
                    MagTriNode2, MagTriNode3, ntrimag, ireinit);
  ...
  dSigmaThTh_dTheta =
      IONOSPHERE_dmatrix(1, IONOSPHERE_numberofnodes, 1, nnodes);
  dSigmaThPs_dTheta =
      IONOSPHERE_dmatrix(1, IONOSPHERE_numberofnodes, 1, nnodes);
  dSigmaPsPs_dTheta =
      IONOSPHERE_dmatrix(1, IONOSPHERE_numberofnodes, 1, nnodes);
  ...
  IONOSPHERE_fgrid(Theta, Psi, X, Y, Z, Radius, IONOSPHERE_numberofnodes,
                   nnodes, fhem);
  ...
  IONOSPHERE_magfac(JR, Theta, Psi, X, Y, Z, Radius, IONOSPHERE_numberofnodes,
                    nnodes, Jmag_x, Jmag_y, Jmag_z, Jmag_R, Thetamag, Psimag,
                    Xmag, Ymag, Zmag, nmag, InMagTri, MagTriNode1, MagTriNode2,
  ...
  IONOSPHERE_magbfield(Bx, By, Bz, Theta, Psi, X, Y, Z, Radius,
                       IONOSPHERE_numberofnodes, nnodes, Bmag_x, Bmag_y, Bmag_z,
                       Thetamag, Psimag, Xmag, Ymag, Zmag, nmag, InMagTri,
                       MagTriNode1, MagTriNode2, MagTriNode3, ntrimag, ireinit);

  FILE *sebData = fopen("seb.dat", "w");

  fprintf(sebData, "nTh=%d, nPh=%d", IONOSPHERE_numberofnodes, nnodes);
  for (int i = 0; i < IONOSPHERE_numberofnodes; i++) {
    for (int j = 0; j < nnodes; j++) {
      fprintf(sebData, "%lf %lf %lf %lf %lf %lf ", JR[i][j], Theta[i][j],
              Psi[i][j], Bx[i][j], By[i][j], Bz[i][j]);
    }
  }
  ...
  IONOSPHERE_conductance(Sigma0, SigmaH, SigmaP, SigmaThTh, SigmaThPs,
                         ...
                         dSigmaPsPs_dPsi, PHI, Theta, Psi, Radius,
  ...
  IONOSPHERE_multigrid(PHI, Sigma0, SigmaH, SigmaP, SigmaThTh, SigmaThPs,
                       SigmaPsPs, dSigmaThTh_dTheta, dSigmaThPs_dTheta,
                       dSigmaPsPs_dTheta, dSigmaThTh_dPsi, dSigmaThPs_dPsi,
                       dSigmaPsPs_dPsi, Theta, Psi, Radius,
  ...
  IONOSPHERE_current(ETh, EPs, Ex, Ey, Ez, Jx, Jy, Jz, Ux, Uy, Uz, PHI,
                     SigmaThTh, SigmaThPs, SigmaPsPs, Bx, By, Bz, Theta, Psi, X,
                     Y, Z, Radius, IONOSPHERE_numberofnodes, nnodes);
  ...
  IONOSPHERE_magvel(PHI, Ux, Uy, Uz, Theta, Psi, X, Y, Z, Radius,
                    IONOSPHERE_numberofnodes, nnodes, Umag_x, Umag_y, Umag_z,
                    Thetamag, Psimag, Xmag, Ymag, Zmag, nmag, InMagTri,
  ...
  if (iout == 1) {
    ...
    for (j = 1; j <= nnodes; j++) {
      for (i = 1; i <= nhemi; i++) {
        fprintf(output_file,
                ...
                (Radius / IONOSPHERE_Radius_Earth) * Z[i][j], Theta[i][j],
                Psi[i][j], SigmaH[i][j], SigmaP[i][j], 1.0e06 * JR[i][j],
                1.0e-03 * PHI[i][j], Ex[i][j], Ey[i][j], Ez[i][j],
    ...
    IONOSPHERE_copy("ionosphere.dat", file_name);
    output_file = fopen(file_name, "w");
    fprintf(output_file, "# Ionospheric Potential \n");
    fprintf(output_file, "# Theta, Psi, x, y, z, SigmaH, SigmaP, JR, PHI, Ex, "
    ...
    for (j = 1; j <= nnodes; j++) {
      for (i = 1; i <= nhemi; i++) {
        fprintf(output_file,
                ...
                (Radius / IONOSPHERE_Radius_Earth) * X[i][j],
                (Radius / IONOSPHERE_Radius_Earth) * Y[i][j],
                (Radius / IONOSPHERE_Radius_Earth) * Z[i][j], SigmaH[i][j],
                SigmaP[i][j], 1.0e06 * JR[i][j], 1.0e-03 * PHI[i][j], Ex[i][j],
                Ey[i][j], Ez[i][j], 1.0e06 * Jx[i][j], 1.0e06 * Jy[i][j],
                1.0e06 * Jz[i][j], 1.0e-03 * Ux[i][j], 1.0e-03 * Uy[i][j],
                1.0e-03 * Uz[i][j]);
      } /* endfor */
      fprintf(output_file, "\n");
    } /* endfor */
  ...
  IONOSPHERE_free_dmatrix(dSigmaThTh_dTheta, 1, IONOSPHERE_numberofnodes, 1,
                          nnodes);
  IONOSPHERE_free_dmatrix(dSigmaThPs_dTheta, 1, IONOSPHERE_numberofnodes, 1,
                          nnodes);
  IONOSPHERE_free_dmatrix(dSigmaPsPs_dTheta, 1, IONOSPHERE_numberofnodes, 1,
                          nnodes);
  IONOSPHERE_free_dmatrix(dSigmaThTh_dPsi, 1, IONOSPHERE_numberofnodes, 1,
                          nnodes);
  IONOSPHERE_free_dmatrix(dSigmaThPs_dPsi, 1, IONOSPHERE_numberofnodes, 1,
                          nnodes);
  IONOSPHERE_free_dmatrix(dSigmaPsPs_dPsi, 1, IONOSPHERE_numberofnodes, 1,
                          nnodes);
