...
void IONOSPHERE_conductance(Sigma0, SigmaH, SigmaP, SigmaThTh, SigmaThPs,
                            SigmaPsPs, dSigmaThTh_dTheta, dSigmaThPs_dTheta,
                            dSigmaPsPs_dTheta, dSigmaThTh_dPsi, dSigmaThPs_dPsi,
                            dSigmaPsPs_dPsi, PHI, Theta, Psi, Radius, nTheta,
                            nPsi) double **Sigma0,
    **SigmaH, **SigmaP, **SigmaThTh, **SigmaThPs, **SigmaPsPs,
    **dSigmaThTh_dTheta, **dSigmaThPs_dTheta, **dSigmaPsPs_dTheta,
    **dSigmaThTh_dPsi, **dSigmaThPs_dPsi, **dSigmaPsPs_dPsi, **PHI, **Theta,
    **Psi, Radius;
int nTheta, nPsi;
{
  int i, j;
    ...
    if (j > 1 && j < nPsi) {
      for (i = 2; i < nTheta; i++) {
        dSigmaThTh_dTheta[i][j] =
            (SigmaThTh[i + 1][j] - SigmaThTh[i - 1][j]) / (2.00 * dd);
        dSigmaThTh_dPsi[i][j] =
            (SigmaThTh[i][j + 1] - SigmaThTh[i][j - 1]) / (2.00 * dd);

        dSigmaThPs_dTheta[i][j] =
            (SigmaThPs[i + 1][j] - SigmaThPs[i - 1][j]) / (2.00 * dd);
        dSigmaThPs_dPsi[i][j] =
            (SigmaThPs[i][j + 1] - SigmaThPs[i][j - 1]) / (2.00 * dd);

        dSigmaPsPs_dTheta[i][j] =
            (SigmaPsPs[i + 1][j] - SigmaPsPs[i - 1][j]) / (2.00 * dd);
        dSigmaPsPs_dPsi[i][j] =
            (SigmaPsPs[i][j + 1] - SigmaPsPs[i][j - 1]) / (2.00 * dd);
      } /* endfor */
    } else if (j == 1) {
      for (i = 2; i < nTheta; i++) {
        dSigmaThTh_dTheta[i][j] =
            (SigmaThTh[i + 1][j] - SigmaThTh[i - 1][j]) / (2.00 * dd);
        dSigmaThTh_dPsi[i][j] =
            (SigmaThTh[i][j + 1] - SigmaThTh[i][nPsi - 1]) / (2.00 * dd);

        dSigmaThPs_dTheta[i][j] =
            (SigmaThPs[i + 1][j] - SigmaThPs[i - 1][j]) / (2.00 * dd);
        dSigmaThPs_dPsi[i][j] =
            (SigmaThPs[i][j + 1] - SigmaThPs[i][nPsi - 1]) / (2.00 * dd);

        dSigmaPsPs_dTheta[i][j] =
            (SigmaPsPs[i + 1][j] - SigmaPsPs[i - 1][j]) / (2.00 * dd);
        dSigmaPsPs_dPsi[i][j] =
            (SigmaPsPs[i][j + 1] - SigmaPsPs[i][nPsi - 1]) / (2.00 * dd);
      } /* endfor */
    } else {
      for (i = 2; i < nTheta; i++) {
        dSigmaThTh_dTheta[i][j] =
            (SigmaThTh[i + 1][j] - SigmaThTh[i - 1][j]) / (2.00 * dd);
        dSigmaThTh_dPsi[i][j] =
            (SigmaThTh[i][2] - SigmaThTh[i][j - 1]) / (2.00 * dd);

        dSigmaThPs_dTheta[i][j] =
            (SigmaThPs[i + 1][j] - SigmaThPs[i - 1][j]) / (2.00 * dd);
        dSigmaThPs_dPsi[i][j] =
            (SigmaThPs[i][2] - SigmaThPs[i][j - 1]) / (2.00 * dd);

        dSigmaPsPs_dTheta[i][j] =
            (SigmaPsPs[i + 1][j] - SigmaPsPs[i - 1][j]) / (2.00 * dd);
        dSigmaPsPs_dPsi[i][j] =
            (SigmaPsPs[i][2] - SigmaPsPs[i][j - 1]) / (2.00 * dd);
      } /* endfor */
    } /* endif */
  } /* endfor */
}
