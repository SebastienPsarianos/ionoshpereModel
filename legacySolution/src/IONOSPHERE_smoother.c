...
double **PHI, **RHO, **SigmaThTh, **SigmaThPs, **SigmaPsPs,
       **dSigmaThTh_dTheta, **dSigmaThPs_dTheta, **dSigmaPsPs_dTheta,
       **dSigmaThTh_dPsi, **dSigmaThPs_dPsi, **dSigmaPsPs_dPsi,
       **Theta, **Psi, Radius;
int nTheta, nPsi;
        ...
        switch (iboundary)
          {
            case 0:
              ...
              for (j=1; j<=nPsi; j++) {
                 PHI[1][j]=0.00;
                 /* PHI[nTheta][j]=0.00; */
                 PHI[nTheta][j]=(PHI[nTheta-1][j]+
                                dd*RHO[nTheta][j]/SigmaThTh[nTheta][j]);
              } /* endfor */
              break;
            ...
            case 2:
              ...
              for (j=1; j<=nPsi; j++) {
                 ...
                 if (j > 1 && j < nPsi) {
                   PHI[nTheta][j]=(kappa_Theta2*(PHI[nTheta-1][jshift]+PHI[nTheta-1][j])
                                  +0.50*dd*kappa_Theta1*(PHI[nTheta-1][jshift]-PHI[nTheta-1][j])
                                  +kappa_Psi2*(PHI[nTheta][j+1]+PHI[nTheta][j-1])
                                  +0.50*dd*kappa_Psi1*(PHI[nTheta][j+1]-PHI[nTheta][j-1])
                                  -dd2*RHO[nTheta][j])/
                                  (2.00*(kappa_Theta2+kappa_Psi2));
                 } else if (j == 1) {
                   PHI[nTheta][j]=(kappa_Theta2*(PHI[nTheta-1][jshift]+PHI[nTheta-1][j])
                                  +0.50*dd*kappa_Theta1*(PHI[nTheta-1][j]-PHI[nTheta-1][jshift])
                                  +kappa_Psi2*(PHI[nTheta][j+1]+PHI[nTheta][nPsi-1])
                                  +0.50*dd*kappa_Psi1*(PHI[nTheta][j+1]-PHI[nTheta][nPsi-1])
                                  -dd2*RHO[nTheta][j])/
                                  (2.00*(kappa_Theta2+kappa_Psi2));
                 } else {
                   PHI[nTheta][j]=(kappa_Theta2*(PHI[nTheta-1][jshift]+PHI[nTheta-1][j])
