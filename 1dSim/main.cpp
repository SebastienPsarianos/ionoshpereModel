...
enum SourceTerm { EXPONENTIAL, CONSTANT, LAPLACE };
enum Algorithm { GAUSS_SEIDEL, JACOBI };

class Solver {
public:
  Solver(int gridSize, double x0, double xMax, double u_x0, double u_xMax,
  ...
  void solve(Algorithm algo) {
    ...
    for (int itt = 0; itt < MAX_ITERATION_NUM; itt++) {
      ...
      // Calculate the residual after each iteration. After the swap, _u[0] will
      // contain the current iteration result regardless of algorithm
      double res = 0;
      for (int x = 1; x < _gridSize - 1; x++) {
        res += std::abs((_u[0][x + 1] + _u[0][x - 1] - 2 * _u[0][x]) / _dx2 -
                        _rho[x]);
      }
      ...
      if (res < RES_THRESHOLD) {
        std::cout << "Simulation converged with " << itt
                  << " iterations and a residual of " << res << std::endl;
        return;
      }
    }
